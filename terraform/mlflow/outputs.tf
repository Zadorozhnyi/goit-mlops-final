output "mlflow_service" {
  ...
  value       = "mlflow-tracking-tracking.${var.target_namespace}.svc.cluster.local:5000"
}

output "mlflow_ui_port_forward" {
  description = "Open the MLflow UI on http://localhost:5000"
  value       = "kubectl port-forward svc/mlflow-tracking-tracking -n ${var.target_namespace} 5000:5000"
}
